...
void initialize(void)
{
	...
	gFragmentShaderObject = setShader("FRAGMENT_SHADER","fragmentShader.glsl",NULL);
	...
	glGenTextures(1, &intermediateOutput);
	glBindTexture(GL_TEXTURE_2D, intermediateOutput);
	glTexStorage2D(GL_TEXTURE_2D, 8, GL_RGBA32F, 512, 512);
...
int LoadGLTextures(GLuint *texture, TCHAR imageResourceId[])
{
	...
	if (hBitmap)
	{
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, bmp.bmWidth, bmp.bmHeight, 0, GL_BGR, GL_UNSIGNED_BYTE, bmp.bmBits);
	}
...

{
	...
	//bind output image
	glBindImageTexture(1, intermediateOutput, 0, GL_FALSE, 0, GL_WRITE_ONLY, GL_RGBA32F);
	//bind input image 
	glBindImageTexture(0, imageInput, 0, GL_FALSE, 0, GL_READ_ONLY, GL_RGBA32F);
	...
	//bind output image
	glBindImageTexture(0, intermediateOutput, 0, GL_FALSE, 0, GL_READ_ONLY, GL_RGBA32F);
	//bind input image 
	glBindImageTexture(1, imageOutput, 0, GL_FALSE, 0, GL_WRITE_ONLY, GL_RGBA32F);
	//dispatch compute shader
	glDispatchCompute(1, 512, 1);
...
void resize(int width, int height)
{
	...
	gOrthoProjectionMatrix = ortho(-100.0, 100.0, -100.0, 100.0, -1.0, 1.0);
