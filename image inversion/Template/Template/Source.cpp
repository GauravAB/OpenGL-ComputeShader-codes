...
void initialize(void)
{
	...
	glUniform1i(glGetUniformLocation(gComputeShaderProgramObject, "img_input"), 0);
	glUniform1i(glGetUniformLocation(gComputeShaderProgramObject, "img_output"), 1);
...
int LoadGLTextures(GLuint *texture, TCHAR imageResourceId[])
{
	...
	if (hBitmap)
	{
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F , 512, 512, 0, GL_BGR, GL_UNSIGNED_BYTE, bmp.bmBits);
	}
