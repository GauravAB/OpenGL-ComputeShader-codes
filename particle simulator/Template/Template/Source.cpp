...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR nCmdLine, int nCmdShow)
{
	...
	WNDCLASSEX wndclass;
	MSG msg;
	TCHAR szAppName[] = TEXT("Particle simulator");
...
void initialize(void)
{
	...
	dt_location = glGetUniformLocation(gComputeShaderProgramObject, "dt");


	

	//create vertex buffer array
	glGenVertexArrays(1, &gVao_render);
	glBindVertexArray(gVao_render);
	...
	glVertexAttribPointer(GAB_ATTRIBUTE_VERTEX, 4 , GL_FLOAT, GL_FALSE, 0, NULL);
...

{
	...
	for (int i = 0; i < 32; i++)
	{
		attractors[i] = vec4(sinf(time*float(i + 4) * 7.5f * 20.0f) * 50.0f,
...
void uninitialize(void)
{
	...
	if (gVao_render)
	{
		glDeleteVertexArrays(1, &gVao_render);
		gVao_render = 0;
	}
	...
	glDeleteProgram(gComputeShaderProgramObject);
	glUseProgram(0);
